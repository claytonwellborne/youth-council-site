...
import { supabase } from "/src/lib/supabase"; // vite accepts /src absolute
import EditorLite from "/src/components/editor/EditorLite";
import { useNavigate, useSearchParams } from "react-router-dom";

const parseTags = (s) => Array.from(new Set((s||"").split(",").map(t=>t.trim()).filter(Boolean).map(t=>t[0].toUpperCase()+t.slice(1).toLowerCase())));
...
export default function PressEditor(){
  ...
  const [tagInput,setTagInput]=useState(""); const [saving,setSaving]=useState(false);

  useEffect(()=>{ (async ()=>{
    if(!postId) return;
    const { data } = await supabase.from('press_posts').select('*').eq('id', postId).maybeSingle();
    if (data){ setPost(data); setTagInput((data.tags||[]).join(', ')); }
  })(); },[postId]);
  ...
  const upsertContrib = async (savedId, status)=>{
    const { data:u } = await supabase.auth.getUser(); const me=u?.user?.id; if(!me||!savedId) return;
  ...
  const save = async (status=post.status)=>{
    ...
    const { data, error } = await q; setSaving(false);
    if (error) return alert(error.message);
    await upsertContrib(data?.id || postId, status);
