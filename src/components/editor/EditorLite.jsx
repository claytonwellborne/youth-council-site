...
export default function EditorLite({ value="", onChange, onUploadAsset }) {
  ...
  const exec = (cmd, arg=null) => { document.execCommand(cmd,false,arg); ref.current?.focus(); onChange?.(ref.current?.innerHTML || ""); };
  const applyBlock = (tag) => { document.execCommand('formatBlock', false, tag); ref.current?.focus(); onChange?.(ref.current?.innerHTML || ""); };
