  ...
  return (
    <div className="rounded-2xl border bg-white overflow-hidden">
      <div className="flex flex-wrap gap-2 p-2 border-b bg-zinc-50">
        ...
        {!showLink && <button className="px-2 py-1 border rounded" onClick={()=>setShowLink(true)}>Link</button>}
        {showLink && (
          <span className="flex items-center gap-1">
            <input className="border rounded px-2 py-1 text-sm" placeholder="https://"
              onKeyDown={(e)=>{ if(e.key==='Enter'){ exec('createLink', e.currentTarget.value); setShowLink(false);} }} />
            <button className="px-2 py-1 border rounded" onClick={()=>setShowLink(false)}>Cancel</button>
          </span>
        )}
      ...
      <div
        ref={ref}
        dir="ltr"
        className="min-h-[320px] p-5 prose max-w-none focus:outline-none"
        contentEditable
